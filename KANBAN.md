...
> 最后更新: 2026-08-15 14:38 CST（v8.3 训练三次启动·数据修复 1337 条·收尾 cron 已挂 15:12）
...
### #1 MLX v8.3 训练迭代（12:58 启动 · ⚠️ iter 50/150 进程退出）
...
- **事故记录**：①首轮 13:07 被 exec 600s 超时杀（iter50 后）②cron 代理 13:43 重启失败（数据混入 27 条 messages 格式坏行）③已修复：git 恢复干净 1310 条 + 转换抢救 27 条 = 1337 条，nohup 脱离重启（14:35 PID 3195）
- **验收标准**：30 题复评 ≥ 90 且 idx23/24 ≥ 70、tag_leak = 0、placeholder_unhandled ≤ 1
- **状态**：训练进程在 iter 50 后退出（semaphore leak warning），checkpoint 已保存（adapters.safetensors 28MB）
- **下一步**：① 检查是否需 resume 训练 50→150 ② 或直接 fuse iter50 → 30 题评估看 idx23/24 改善 ③ 达标则切 8960
