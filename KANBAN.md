...
>
> > 最后更新: 2026-08-18 04:40 CST（心跳巡检：8 条蒸馏候选审核完结全部 reject + KANBAN 进行中区段清理）
...
### #5 v8.10 训练计划（等 v2.1 复评基线）🔄
- **前置阻塞**：v2.1 复评流水线未完成——评估器脚本 `eval-baziqa-v2.py` 随 `.openclaw/tmp/giant-shoulders/` 清理丢失，v8.7 v2.1 基线数据缺失
- **待办**：①重建 eval-baziqa-v2.py（v2.1 规范：去「如 A」示例 + max_tokens 200 + 五级提取 + 空提取重试 + raw 审计）②重跑 v8.7 v2.1 全量 BaziQA ③达标（≥52%）后执行 v8.10：Qwen2.5-3B base 直训 + v8.7 最优配方（推理链 246/自由问答 364，lr 5e-6，200 iters）+ 补「只输出字母」格式样本
- **经验固化**：推理链与自由问答 1:1 配比最优；自由问答过多稀释推理、过少损失泛化

### #6 staging 积压审核（152 条 pending）🔄
- **构成**：KB-tcm-* 139 条（08-09 批量 QA）+ 路总流年班合婚技法 entry-* 4 条 + lm-* 路大师专利 5 条 + fb_* 反馈蒸馏 2 条 + 其他
- **待办**：逐批比对 kb_formal / authority-tcm 权威库，重复拒绝、有效 promote，清空积压
- **注意**：KB-tcm-* 139 条需先确认是否走 tcm-agent 权威库流程，避免误拒
