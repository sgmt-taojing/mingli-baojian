...
function showWechatGuide(){
  ...
  guide.innerHTML = '<div style="background:linear-gradient(135deg,var(--ink3),var(--ink));border:1px solid rgba(201,168,76,.3);border-radius:16px;padding:28px 24px;max-width:320px;text-align:center">'+
    '<div style="font-size:48px;margin-bottom:12px">🎤</div>'+
    '<h3 style="color:var(--gold);font-size:17px;margin-bottom:10px;letter-spacing:2px">语音功能需要浏览器支持</h3>'+
    '<p style="color:var(--paper3);font-size:13px;line-height:1.8;margin-bottom:16px">微信内置浏览器不支持语音识别。<br>请点击右上角 <b style="color:var(--gold)">···</b> → 选择 <b style="color:var(--gold)">在浏览器中打开</b> 即可使用语音算命功能。</p>'+
    '<button onclick="this.parentElement.parentElement.remove()" style="background:linear-gradient(135deg,var(--gold),var(--gold3));border:none;color:var(--ink3);padding:10px 32px;border-radius:20px;font-size:14px;cursor:pointer;font-family:inherit">我知道了</button>'+
    '</div>';
...
function showPlayer(label, onReady){
  ...
  player.style.cssText = 'position:fixed;bottom:70px;left:12px;right:12px;background:linear-gradient(135deg,var(--ink3),var(--ink));border:1px solid rgba(201,168,76,.3);border-radius:12px;padding:12px 16px;display:flex;align-items:center;gap:12px;z-index:9998;box-shadow:0 4px 20px rgba(0,0,0,.4)';
  player.innerHTML = '<div style="font-size:20px">🔊</div>'+
    ...
    '<button onclick="mobileStopSpeak()" style="background:none;border:none;color:var(--paper3);font-size:18px;cursor:pointer;padding:4px">✕</button>';
...
function showListeningOverlay(){
  ...
  ov.innerHTML = '<div style="width:80px;height:80px;border-radius:50%;background:linear-gradient(135deg,var(--gold),var(--gold3));display:flex;align-items:center;justify-content:center;font-size:36px;animation:pulse 1.5s infinite;margin-bottom:16px">🎤</div>'+
    '<div id="voiceText" style="color:var(--gold);font-size:14px;max-width:280px;text-align:center;min-height:20px">正在聆听...</div>'+
    '<div style="color:var(--paper3);font-size:12px;margin-top:12px">说出您的出生日期，如"1979年6月15日酉时"</div>'+
...
// console.warn('🎙️ 移动端语音模块已加载 (微信:' + isWechat + ' 移动:' + isMobile + ')');
})();
