...
function _getUnifiedReport(modId,data){
  ...
  if(modId==='ziwei'){
    ...
    var bodyStar={'金':'武曲/天柱','木':'贪狼/天机','水':'巨门/太阴','火':'廉贞/太阳','土':'紫微/天府'}[zele]||'紫微';
    var lifeStar={'金':'七杀','木':'天机','水':'破军','火':'廉贞','土':'紫微'}[zele]||'紫微';
    var mingGong='';var siblings='';var spouse='';var career='';var wealth='';var health='';
    // 十二宫流年推断(简化版，基于五行+日主)
    var stars12=['紫微','天机','太阳','武曲','天同','廉贞','天府','太阴','贪狼','巨门','天相','天梁','七杀','破军'];
    var juZi=['子宫','丑宫','寅宫','卯宫','辰宫','巳宫','午宫','未宫','申宫','酉宫','戌宫','亥宫'];
    var mainStar=stars12[(zy+zm2+zd)%14];
    var bodyPalace=juZi[(zh)%12];
    var mingStar2=stars12[(zy+zm2*3+zd*7)%14];
    var spStar=stars12[(zy+7)%14];
    var caStar=stars12[(zy+zm2+9)%14];
    var caStar2=stars12[(zy+zm2+5)%14];
    var offStar=stars12[(zy+zm2+11)%14];
    var trStar=stars12[(zy+zm2+2)%14];
    var paStar=stars12[(zy+zm2+8)%14];
    var foStar=stars12[(zy+zm2+4)%14];
    // 各宫吉凶
    var starDesc={
      '紫微':'帝星·领导力强·主观固执','天机':'智多星·善于谋划·神经敏感','太阳':'贵星·热情大方·男主父旺女主大吉','武曲':'财星·刚毅果决·财录有成','天同':'福星·乐观享福·安于现状','廉贞':'囚星·次桃花·干练多争','天府':'库星·稳重保守·财蓄有成','太阴':'母星·温柔内敛·女主母旺','贪狼':'桃花·多才多艺·欲望强','巨门':'暗星·口才好但多是非','天相':'印星·循规蹈矩·助人助己','天梁':'荫星·正直有原则·老成持重','七杀':'将星·刚毅勇敢·波动大','破军':'耗星·破旧立新·变化多端'
    };
    var zr='━━━ 紫微斗数完整排盘报告 ━━━\n\n';
    zr+='【生辰】'+zsBirth+'\n';
    zr+='【日主】'+zdm+'('+zele+')\n';
    zr+='【五行】金'+zwx['金']+' 木'+zwx['木']+' 水'+zwx['水']+' 火'+zwx['火']+' 土'+zwx['土']+(zlack.length?' · 缺'+zlack.join('、'):' · 五行俱全')+'\n';
    zr+='【命主星】'+mainStar+' → '+starDesc[mainStar]+'\n';
    zr+='【身主星】'+mingStar2+' → '+starDesc[mingStar2]+'\n';
    zr+='【身宫】'+bodyPalace+'\n\n';
    zr+='━━━ 一、十二宫详细排盘 ━━━\n';
    zr+='┌─────────────────────────┐\n';
    zr+='│ 1.命宫 → '+mainStar+'('+starDesc[mainStar].split('·')[0]+')\n';
    zr+='│ 2.兄弟宫 → '+stars12[(zy+1)%14]+'('+starDesc[stars12[(zy+1)%14]].split('·')[0]+')\n';
    zr+='│ 3.夫妻宫 → '+spStar+'('+starDesc[spStar].split('·')[0]+')\n';
    zr+='│ 4.子女宫 → '+offStar+'('+starDesc[offStar].split('·')[0]+')\n';
    zr+='│ 5.财帛宫 → '+caStar+'('+starDesc[caStar].split('·')[0]+')\n';
 zr+='│ 6.疾厄宫 → '+stars12[(zy+3)%14]+'('+starDesc[stars12[(zy+3)%14]].split('·')[0]+')\n';
    zr+='│ 7.迁移宫 → '+trStar+'('+starDesc[trStar].split('·')[0]+')\n';
    zr+='│ 8.仆役宫 → '+stars12[(zy+5)%14]+'('+starDesc[stars12[(zy+5)%14]].split('·')[0]+')\n';
    zr+='│ 9.官禄宫 → '+caStar2+'('+starDesc[caStar2].split('·')[0]+')\n';
    zr+='│ 10.田宅宫 → '+stars12[(zy+7)%14]+'('+starDesc[stars12[(zy+7)%14]].split('·')[0]+')\n';
    zr+='│ 11.福德宫 → '+foStar+'('+starDesc[foStar].split('·')[0]+')\n';
    zr+='│ 12.父母宫 → '+paStar+'('+starDesc[paStar].split('·')[0]+')\n';
    zr+='└─────────────────────────┘\n\n';
    zr+='━━━ 二、核心宫位深度解读 ━━━\n';
    ...
    zr+='【夫妻宫 · '+spStar+'】\n'+starDesc[spStar]+'\n'+(spStar==='紫微'?'配偶能干有主见，宜充分沟通。':spStar==='太阴'?'配偶温柔体贴，女主贤淑男主得贤内助。':spStar==='天府'?'配偶稳重可靠，善于理财。':spStar==='贪狼'?'配偶多才多艺但需注意桃花。':spStar==='武曲'?'配偶刚毅果决，事业心强。':spStar==='天同'?'配偶乐观温和，感情甜蜜。':spStar==='七杀'||spStar==='破军'?'感情波动大，需多包容。':'配偶性格'+starDesc[spStar].split('·')[1])+'\n\n';
    ...
    ages.forEach(function(a){zr+='· '+a+'-'+(a+9)+'岁 → '+stars12[(zy+a)%14]+'运 → '+starDesc[stars12[(zy+a)%14]].split('·')[0]+'\n';});
    zr+='\n';
    zr+='━━━ 五、五行调候建议 ━━━\n';
...
// R206: 精确节气表（2020-2030，每月「节」日期，用于月柱判断）
  // 12个月每月的「节」日期（立春/惊蛰/清明/立夏/芒种/小暑/立秋/白露/寒露/立冬/大雪/小寒）
  var JIE_DATES={
    2020:[4,6,5,5,6,7,7,8,8,8,7,6],
    2021:[3,5,5,5,5,7,7,8,8,7,7,5],
    2022:[4,6,5,5,6,7,7,8,8,7,7,5],
    2023:[4,6,5,6,6,7,8,8,8,8,7,6],
    2024:[4,5,5,5,5,7,7,7,8,7,7,6],
    2025:[3,5,5,5,5,7,7,7,8,7,7,5],
    2026:[4,6,5,5,6,7,7,8,8,7,7,5],
    2027:[4,6,5,5,6,7,7,8,8,7,7,5],
    2028:[4,5,5,5,5,7,7,7,8,7,7,5],
    2029:[4,5,5,5,5,7,7,7,8,7,7,5],
    2030:[4,5,5,5,5,7,7,7,8,7,7,5]
  };
  // 确定月柱地支索引（寅=1月, 卯=2月, ..., 丑=12月）
  // 节气月：立春(寅)=1月, 惊蛰(卯)=2月, 清明(辰)=3月, 立夏(巳)=4月, 芒种(午)=5月, 小暑(未)=6月, 立秋(申)=7月, 白露(酉)=8月, 寒露(戌)=9月, 立冬(亥)=10月, 大雪(子)=11月, 小寒(丑)=12月
  var monthIdx=0; // 0=寅月(1月), 1=卯月(2月), ..., 11=丑月(12月)
  var jieDates=JIE_DATES[y];
  if(jieDates){
    // 立春日期约在2月4日左右，1月用丑月
    var licun=jieDates[0]; // 立春日
    if(m===1 || (m===2 && d<licun)){
      monthIdx=11; // 丑月（12月）
    }else{
      // 从2月开始找
      for(var i=0;i<11;i++){
        var jieMonth=i+2; // 节气对应的公历月
        if(jieMonth>12) jieMonth-=12;
        var jieDay=jieDates[i+1]||jieDates[0]; // 下一节气日
        // 实际上 jieDates[0]=立春(2月), [1]=惊蛰(3月), [2]=清明(4月)...
        // i=0 → 立春(2月) → 寅月(1月)
        // i=1 → 惊蛰(3月) → 卯月(2月)
        // 月份= i+2, 节日 = jieDates[i]
      }
      // 简化：根据公历月和节日判断
      // jieDates[0]=立春(2月4日), [1]=惊蛰(3月6日), [2]=清明(4月5日), [3]=立夏(5月5日), [4]=芒种(6月6日), [5]=小暑(7月7日), [6]=立秋(8月7日), [7]=白露(9月8日), [8]=寒露(10月8日), [9]=立冬(11月7日), [10]=大雪(12月7日), [11]=小寒(1月5日)
      var monthJie=[[2,jieDates[0]],[3,jieDates[1]],[4,jieDates[2]],[5,jieDates[3]],[6,jieDates[4]],[7,jieDates[5]],[8,jieDates[6]],[9,jieDates[7]],[10,jieDates[8]],[11,jieDates[9]],[12,jieDates[10]],[1,jieDates[11]]];
      monthIdx=11; // 默认丑月
      for(var i=0;i<12;i++){
        var jm=monthJie[i][0],jd=monthJie[i][1];
        if(m===jm && d>=jd){monthIdx=i;break;}
        if(m===jm && d<jd){monthIdx=(i+11)%12;break;}
      }
    }
...
function _paipan(y,m,d,h){
  // R206: 精确节气表（2020-2030，±1天精度）替代 d>=15 粗略判断
  var JIEQI={2020:[6,4,20,5,20,5,21,7,22,8,22,8,23,9,23,8,23,8,22,7,22,7,22,7],
  2021:[5,3,20,5,20,5,21,7,22,8,23,8,23,9,23,8,23,8,22,7,22,7,22,7],
  2022:[5,4,20,5,21,6,21,7,23,8,23,8,23,9,23,8,23,8,22,7,22,7,22,7],
  2023:[6,4,21,5,21,6,22,7,23,8,24,8,24,9,24,8,24,8,23,7,22,7,22,7],
  2024:[6,4,20,5,20,5,21,7,22,8,22,8,23,9,23,8,23,8,22,7,22,7,22,7],
  2025:[5,4,20,5,20,6,21,7,22,8,23,8,23,9,23,8,23,8,22,7,22,7,22,7],
  2026:[5,4,20,5,21,6,21,7,23,8,23,8,23,9,23,8,23,8,22,7,22,7,22,7],
  2027:[5,4,20,5,21,6,21,7,23,8,23,8,23,9,23,8,23,8,22,7,22,7,22,7],
  2028:[6,4,20,5,20,5,21,7,22,8,22,8,22,9,22,8,22,8,22,7,21,7,21,7],
  2029:[5,4,20,5,20,5,21,7,22,8,22,8,23,9,23,8,23,8,22,7,22,7,22,7],
  2030:[5,4,20,5,20,6,21,7,22,8,23,8,23,9,23,8,23,8,22,7,22,7,22,7]};
  var jq=JIEQI[y],monthIdx=0;
  if(jq){
    for(var i=0;i<12;i++){
      var jieDay=jq[i*2];
      if(m===i+1){
        if(d>=jieDay) monthIdx=i;
        else if(i>0) monthIdx=i-1;
        else monthIdx=11;
        break;
      }
      if(m===12&&d>=jq[22]) monthIdx=11;
    }
  }else{monthIdx=Math.floor((m-1)*2+(d>=15?1:0));}
  var tg=['甲','乙','丙','丁','戊','己','庚','辛','壬','癸'];
  var dz=['子','丑','寅','卯','辰','巳','午','未','申','酉','戌','亥'];
  const yc=(m<2||(m===2&&d<4))?y-1:y;
  const yS=tg[((yc-4)%10+10)%10],yB=dz[((yc-4)%12+12)%12];
  const mB=dz[((monthIdx+2)%12+12)%12];
  const yI=((yc-4)%10+10)%10;
  const mI=(yI*2+((monthIdx+2)%12+12)%12)%10;
  const mS=tg[mI];
  const ep=new Date(1900,0,1),tg2=new Date(y,m-1,d);
  const dd=Math.floor((tg2-ep)/86400000);
  const dS=tg[((dd%10)+10)%10],dB=dz[((dd+10)%12+12)%12];
  const hI=Math.floor((h+1)/2)%12;
  const hB=dz[hI],hI2=(dS?tg.indexOf(dS):0);
  const hS=tg[(hI2*2+hI)%10];
  // R206: 十神计算
  ...
  const em={'甲':'木','乙':'木','丙':'火','丁':'火','戊':'土','己':'土','庚':'金','辛':'金','壬':'水','癸':'水','子':'水','丑':'土','寅':'木','卯':'木','辰':'土','巳':'火','午':'火','未':'土','申':'金','酉':'金','戌':'土','亥':'水'};
  [yS+yB,mS+mB,dS+dB,hS+hB].forEach(g=>{wx[em[g[0]]]++;wx[em[g[1]]]++;});
  // R206: 大运方向（阳年男/阴年女顺排，阴年男/阳年女逆排）
  var yYang=(yc%2===0);
  var sexMale=true; // 默认男命，外部可传 sex 参数覆盖
  var forward=(yYang===sexMale);
  var dayunStart=Math.floor((10-(h%12))/2); // 简化起运岁
  var dayunList=[];
  var curM=monthIdx;
  for(var i=0;i<8;i++){
    curM=forward?(curM+1)%12:((curM+11)%12);
  ...
  var nayinIdx=((tg.indexOf(yS)*10+dz.indexOf(yB))%30+30)%30;
  return{day_master:dS+em[dS],pillars:{'年':yS+yB,'月':mS+mB,'日':dS+dB,'时':hS+hB},
    wuxing_count:wx,wuxing_lack:Object.keys(wx).filter(k=>wx[k]===0),
    shishen:shishenMap,dayun:dayunList,nayin:nayinMap[nayinIdx],
    jieqi_month:monthIdx+1};
}
