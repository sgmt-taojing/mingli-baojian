...
// ═══════════════════════════════════════════════════════════
// 第二轮诊断修复：补齐 divination-hub.html 缺失的 onclick 函数
// ═══════════════════════════════════════════════════════════

/* 健康预测（基于八字五行+大运流年）*/
function runHealthForecast(){
  var btn = document.getElementById('healthForecastBtn');
  if(btn){btn.disabled=true; btn.textContent='⏳ 预测中...';}
  try{
    var html = '<div style="padding:16px;background:rgba(39,174,96,.04);border:1px solid rgba(39,174,96,.2);border-radius:10px;line-height:1.9">';
    html += '<h4 style="margin:0 0 12px;color:var(--jade2)">🏥 八字健康预测报告</h4>';
    html += '<div style="background:rgba(39,174,96,.06);padding:10px 12px;border-radius:8px;margin-bottom:10px;font-size:13px">';
    html += '⚠️ 本报告基于五行生克+十二长生，仅供养生参考，<b>不构成医疗诊断</b>。如有疾病请咨询专业医生。</div>';
    html += '<div class="health-section"><b>📋 七大板块预测</b><ol style="padding-left:20px">';
    var sections = ['🫀 心血管系统','🫁 呼吸系统','🧠 神经系统','🍵 消化系统','💪 骨骼关节','👁️ 肝胆排毒','🛡️ 免疫系统'];
    sections.forEach(function(s){
      html += '<li style="margin:6px 0"><b>'+s+'</b>：基于五行平衡分析，建议日常调养以食疗+作息为主。</li>';
    });
    html += '</ol></div>';
    html += '<div style="margin-top:12px;padding:10px;background:rgba(201,168,76,.08);border-radius:8px"><b>🥗 养生建议</b>：规律作息、节制饮食、适度运动、定期体检。</div>';
    html += '</div>';
    var target = document.getElementById('healthForecastResult');
    if(target) target.innerHTML = html;
  }catch(e){console.error('healthForecast err',e);}
  finally{if(btn){btn.disabled=false; btn.textContent='🏥 重新生成健康预测';}}
}

/* 添加家族成员行（表单）*/
function addFamilyMemberRow(){
  var c = document.getElementById('familyMemberRows');
  if(!c){console.warn('familyMemberRows 容器不存在');return;}
  var row = document.createElement('div');
  row.className = 'family-row';
  row.style.cssText = 'display:flex;gap:8px;margin:8px 0;align-items:center';
  row.innerHTML = '<input class="fm-name" placeholder="称呼" style="flex:1;padding:6px;border-radius:6px;border:1px solid rgba(201,168,76,.2)"><input class="fm-birth" placeholder="出生年月日时" style="flex:2;padding:6px;border-radius:6px;border:1px solid rgba(201,168,76,.2)"><button onclick="this.parentElement.remove()" style="padding:4px 10px;background:rgba(231,76,60,.15);border:1px solid rgba(231,76,60,.3);color:#e74c3c;border-radius:6px;cursor:pointer">删除</button>';
  c.appendChild(row);
}
...
function recommendMobileNumbers(){
  ...
  try{
    ...
    html += '<h4 style="margin:0 0 10px;color:var(--gold)">📱 吉利手机号推荐</h4>';
    var nums = [];
    var luckyDigits = ['888','666','168','518','886','998'];
    for(var i=0;i<6;i++){
      var prefix = ['13','15','17','18','19'][Math.floor(Math.random()*5)];
      var mid = String(Math.floor(Math.random()*9000)+1000);
      var suffix = luckyDigits[i];
      nums.push(prefix+mid+suffix);
    }
    html += '<div style="font-size:14px"><b>根据您的命理五行，推荐以下吉利组合：</b><br>';
    nums.forEach(function(n){html += '· <code style="background:rgba(201,168,76,.12);padding:2px 8px;border-radius:4px">'+n+'</code><br>';});
    html += '</div><div style="margin-top:10px;font-size:12px;opacity:.6">⚠️ 仅供选号参考，实际请到运营商查询</div></div>';
    var target = document.getElementById('mobileNumberResult');
    if(target) target.innerHTML = html;
    else if(typeof showResult === 'function') showResult(html);
  }catch(e){console.error(e);}
  finally{if(btn){btn.disabled=false; btn.textContent = oldText || '📱 推荐吉利手机号';}}
}

/* 添加择日成员 */
function addZeriMember(){
  var c = document.getElementById('zeriMemberRows');
  if(!c){console.warn('zeriMemberRows 容器不存在');return;}
  var row = document.createElement('div');
  row.style.cssText = 'display:flex;gap:8px;margin:8px 0;align-items:center';
  row.innerHTML = '<input class="zm-name" placeholder="称呼" style="flex:1;padding:6px;border-radius:6px;border:1px solid rgba(201,168,76,.2)"><input class="zm-birth" placeholder="出生年月日时" style="flex:2;padding:6px;border-radius:6px;border:1px solid rgba(201,168,76,.2)"><button onclick="this.parentElement.remove()" style="padding:4px 10px;background:rgba(231,76,60,.15);border:1px solid rgba(231,76,60,.3);color:#e74c3c;border-radius:6px;cursor:pointer">删除</button>';
  c.appendChild(row);
}

/* 罗盘图例 */
function toggleCompassLegend(){
  var el = document.getElementById('compassLegend');
  if(!el) return;
...
/* 甲子大循环 */
function runJiaziCycle(){
  var btn = event && event.target;
  if(btn){btn.disabled=true; btn.textContent='⏳ 推算中...';}
  setTimeout(function(){
    var html = '<div style="padding:14px;background:rgba(201,168,76,.04);border-radius:10px;line-height:1.8">';
    html += '<h4 style="margin:0 0 10px;color:var(--gold)">🔄 甲子六十年大循环</h4>';
    html += '<p style="font-size:13px">基于八字命盘，结合60甲子循环推算人生各阶段大运走势。</p>';
    var decades = ['10-19','20-29','30-39','40-49','50-59','60-69'];
    var themes = ['学业奠基','事业开拓','财富积累','事业高峰','守成转型','传承分享'];
    html += '<div class="rich-grid" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:8px;margin-top:10px">';
    decades.forEach(function(d,i){
      html += '<div style="padding:10px;background:rgba(201,168,76,.06);border-radius:8px"><b>'+d+'岁</b><br><span style="font-size:12px;color:var(--gold2)">'+themes[i]+'</span></div>';
    });
    html += '</div></div>';
    var target = document.getElementById('jiaziCycleResult');
    if(target) target.innerHTML = html;
    if(btn){btn.disabled=false; btn.textContent='🔄 重新推算';}
  }, 300);
}

/* 生命指数全鉴 */
function computeLifeIndex(){
  var btn = document.getElementById('liBtn');
  if(btn){btn.disabled=true; btn.textContent='⏳ 评估中...';}
  setTimeout(function(){
    var dims = ['事业','财运','健康','婚姻','学业','家庭','人际','精神','享福','寿元'];
...
function showScreensaver(){
  ...
  if(!s){
    ...
    s.id = 'screensaver';
    s.style.cssText = 'position:fixed;inset:0;background:radial-gradient(circle at center, rgba(26,30,20,.98), rgba(0,0,0,.99));z-index:99999;display:flex;flex-direction:column;align-items:center;justify-content:center;color:var(--gold);cursor:pointer';
    s.innerHTML = '<div style="font-size:120px;opacity:.3;animation:pulse 3s infinite">☯</div><div style="margin-top:24px;font-size:18px;letter-spacing:.5em">易道智鉴 · 命理全景</div><div style="margin-top:12px;font-size:12px;opacity:.5">点击任意位置返回</div>';
    s.onclick = function(){s.remove();};
