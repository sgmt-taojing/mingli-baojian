...
function tzSwitchTab(name){
  const allTabs = ['report','selftest','plan','food','qigong','prescription','bazi','checkin','wuxing'];
...
function tzGeneratePlan(){
  ...
  const prompt = '你是中医养生专家。请为"' + cons.name + '"体质的人生成一份综合养生方案。\n';
  ...
  fetch((typeof apiBase!=='undefined'?apiBase:'http://127.0.0.1:8900')+'/v1/chat/completions',{
    method:'POST',
...
function tzRenderFood(filter){
  ...
  if(extraHtml){
    let cur = document.getElementById('tzFoodList').innerHTML;
...
function tzAnalyzeBazi(){
  ...
  let html='<div class="tz-card-lg">'+
    ...
    '<div class="tz-grid-auto">'+
      ...
      '<div class="tz-card"><div class="tz-gold-12-mb6">👅 五味</div><div class="tz-text-14">'+wuData.taste+'味入'+wuData.organs[0]+'</div></div>'+
...
function tzSubmitCheckin(){
  ...
  const data = {date:today,items:items,note:note};
...
function tzSelectWuxing(key){
  ...
  let html='<div class="tz-card-lg">'+
    ...
    '</div>'+
    '<div class="tz-grid-auto">'+
      '<div class="tz-card"><div class="tz-gold-12-mb6">👁 对应</div><div class="tz-text-14">'+d.sense+' · '+d.tissue+'</div></div>'+
      '<div class="tz-card"><div class="tz-gold-12-mb6">⚠ 症状</div><div class="tz-text-12">'+d.symptoms.slice(0,4).join('、')+'</div></div>'+
    '</div>'+
    '<div class="tz-tip-success">'+
      '<div class="tz-success-title">🌿 养生食物</div>'+
