...
// ==================== 数据存储 ====================
var STORAGE_KEY = 'mingli_shop_data';
var UPLOADS_KEY = 'mingli_shop_uploads';
...
function loadData() {
  ...
  // 首次加载从 SHOP_DATA 初始化
  if (window.SHOP_DATA) {
    var data = JSON.parse(JSON.stringify(window.SHOP_DATA));
    saveData(data);
    return data;
  }
...
function loadUploads() {
  var stored = localStorage.getItem(UPLOADS_KEY);
  if (stored) {
    try { return JSON.parse(stored); } catch(e) {}
  }
  return {};
...
function showSection(name) {
  ...
  var section = document.getElementById('shop-section-' + name);
  if (section) section.classList.add('active');
  
  document.querySelectorAll('.shop-admin-nav a').forEach(function(a) {
    a.classList.remove('active');
  });
  var navLink = document.querySelector('.shop-admin-nav a[data-shop-section="' + name + '"]');
...
function renderDashboard() {
  ...
  var active = products.filter(function(p) { return p.status !== 'offline'; }).length;
  var offline = products.filter(function(p) { return p.status === 'offline'; }).length;
  var categories = shopData.categories.length;
  
  var el = document.getElementById('shop-dashboard-stats');
  if (!el) return;
  el.innerHTML = 
...
function renderProductTable() {
  var tbody = document.getElementById('shop-product-tbody');
  if (!tbody) return;
  ...
  products.forEach(function(p) {
    var cat = getCategoryName(p.categoryId);
    var subcat = getSubcategoryName(p.categoryId, p.subcategoryId);
...
function getFilteredProducts() {
  var products = shopData.products.slice();
...
function editProduct(id) {
  var p = shopData.products.find(function(x) { return x.id === id; });
  if (!p) return;
  editingProductId = id;
...
function saveProduct() {
  ...
  if (!form.name) { alert('请输入商品名称'); return; }
  if (!form.categoryId) { alert('请选择分类'); return; }
  
  if (editingProductId) {
    // 编辑
    var idx = shopData.products.findIndex(function(p) { return p.id === editingProductId; });
    if (idx >= 0) {
...
function getFormData() {
  var images = getImagePreviews().filter(function(x) { return x; });
  return {
...
function populateCategorySelects() {
  var selects = ['prod-category', 'shop-category-filter'];
...
function onCategoryChange() {
  var catId = document.getElementById('prod-category').value;
  var subSel = document.getElementById('prod-subcategory');
  subSel.innerHTML = '<option value="">-- 选择子分类 --</option>';
  if (!catId) return;
  var cat = shopData.categories.find(function(c) { return c.id === catId; });
  if (cat && cat.subcategories) {
    cat.subcategories.forEach(function(sc) {
      subSel.innerHTML += '<option value="' + sc.id + '">' + sc.name + '</option>';
    });
  }
}

function renderCategoryList() {
  var el = document.getElementById('shop-category-list');
  if (!el) return;
  ...
  var html = '';
  shopData.categories.forEach(function(c) {
    var subCount = c.subcategories ? c.subcategories.length : 0;
...
function editCategory(id) {
  var c = shopData.categories.find(function(x) { return x.id === id; });
  if (!c) return;
...
function saveCategory() {
  ...
  var subText = document.getElementById('cat-sub-list').value.trim();
  
  if (!name) { alert('请输入分类名称'); return; }
  if (!icon) { alert('请输入分类图标（emoji）'); return; }
  ...
  if (editingCategoryId) {
    var idx = shopData.categories.findIndex(function(c) { return c.id === editingCategoryId; });
...
function deleteCategory(id) {
  ...
  if (!c) return;
  // 检查是否有商品使用此分类
  var usedCount = shopData.products.filter(function(p) { return p.categoryId === id; }).length;
  var msg = '确定要删除分类「' + c.name + '」吗？';
...
// ==================== 图片上传 ====================
var imagePreviews = [null, null, null, null, null];
...
function handleFiles(files) {
  Array.from(files).forEach(function(file) {
    ...
    if (slot < 0) { alert('最多上传5张图片'); return; }
...
function renderImagePreviews() {
  ...
  var hasImages = imagePreviews.some(function(x) { return x; });
...
// ==================== 数据导入导出 ====================
function exportData() {
  var json = JSON.stringify(shopData, null, 2);
    ...
    reader.onload = function(e) {
      try {
        ...
        if (!data.products || !data.categories) {
          alert('JSON格式不正确，需要包含 products 和 categories 字段');
          return;
        }
...
function bindEvents() {
  ...
  // 分类切换时更新子分类
  var catSelect = document.getElementById('prod-category');
  if (catSelect) {
