## 2026-09-20 20:55 · 公众号采集周报 week-20260921 补跑：观音香会/重阳/霜降/易经 4 主题 40 篇（蒸馏流程首次单轮执行）

- **采集**：单轮 4 关键词串行 × 前 10 条 = 40 篇（35 个来源）；autoglm 单次调用、约 4 分钟完成，无重试。系 09-14 网关重启中断 / 09-17 超时两次异常后的修复补跑。
- **落盘**：knowledge/collected/2026-09/week-20260921/ —— 4 采集文档（观音香会/重阳法会/霜降养生/易经讲座）+ fodao-activity-calendar.md（并入 week-20260914 批次要点）+ weekly-report.md。
- **关键日程**：观音香会九月十九 = 2026-10-28（农历引擎双库校验，修正 10-29 估算）；重阳/九皇法会 = 10-18 / 10-10~10-18（2026 预热已现）；霜降 = 10-23（内容窗口开启）；中秋 09-25 举办地已确认。
- **修复沉淀**：parse_collect.py 补「`\|` 转义列错位」预归一化（14 行受影响 → 复验 40/40）；原始结果归档 .openclaw/tmp/collect-w37/archive-20260920/。

## 2026-09-20 17:05 · R824 T1 批 2：滴天髓干支义理 20 条入库 + status 教训落地双脚本

- **入库通道补 status 字段**（R823 教训落地）：distill-backlog-ingest.py 与 bazi-t1-batch1.py 的 INSERT 均补 status='formal'，后续新通道不再产生 NULL status。
- **T1 批 2（KB-BZ2）20 条一次全入**（scripts/bazi-t1-batch2.py，SRC-DITIANSUI-T1B2 / trust 0.85）：十干体象十义（甲参天/乙柔韧/丙猛烈/丁柔中/戊固重/己卑湿/庚带煞/辛软弱/壬通河/癸至弱，每干含气质之辨与取用正路）+ 地支藏干宅/六合合化合绊/三刑恩害/六冲去留/拱夹虚神/顺逆之机/寒暖燥湿/中和流通/体用之辨——每条标注《滴天髓》任注出处。
- **检索验收**：「珠玉蒙尘」「堤防成」等 4 字词均命中 KB-BZ2 新条；fts5 中 KB-BZ2 = 20。
- **终态**：主表=fts5=316,823，bazi 域 623→643，五红线全绿。

## 2026-09-20 15:05 · R823 项目边界检查 + 命理知识/规划推送命理宝鉴（用户令）

- **边界检查（全绿）**：①命理知识唯一承载=命理宝鉴 kb_formal；②family 镜像消费端 20,752 条为合法下发（R747 双字段边界生效，参考域/tcm 内化不出站）；③tcm-agent 库无八字存量，mingli-cross 140 条已迁入；④ai-vision/video 等无命理渗漏。
- **status 字段暗病修复（重要）**：全库 2,248 条 status=NULL（各入库通道漏填），被镜像导出条件 `status IN (formal/active/...)` 整体排除——含 T1 批 1 的 20 条 KB-BZ1（镜像实测 0 条 KB-BZ1）。分诊处置：1,836 条有内容有 trust 转 formal、766 条正规条目回收（目录/字幕类误伤）、2 条 CSS 残渣留 deprecated；终态 NULL 归零。
- **命理知识推送**：手动触发 distill-mingli-outbound → 新镜像 21,142 条（+390），KB-BZ1 20/20 全部入站，smart-home-family 落地确认；registry total=10,507。
- **建设规划落位**：docs/T1-BAZI-ROADMAP.md（八字补库十批规划：批 1 子平真诠✅、批 2 滴天髓、批 3 穷通宝鉴调候、批 4 三命通会神煞、批 5 渊海子平…每批 20 条五红线全检）。
- **终态**：KB 316,803（status 分 incorporate formal 275,943/active 37,569/published 2,610）；五红线全绿；镜像链路含 T1 增量。
- 教训：**新增入库通道必须填 status 字段**（本批 5 个通道漏填 2,248 条），distill-backlog-ingest.py 等脚本下轮补 status 字段写入。

## 2026-09-20 15:10 · R822 任务板 T1 开工：八字补库第一批 20 条（子平真诠核心概念体系）

- **T1 现状盘点**：bazi 域 603 条；子平真诠/滴天髓/穷通内容关联 945-1309 条（多为 r39/r45 十神应用条目）；十干×十二月调候矩阵粗匹配 120/120 全覆盖——真缺口在《子平真诠》核心概念的定义级条目：调候用神取用/从格取用/格局成败救应/用神纯杂/大运干支分工/流年太岁刑冲六项检索为 0。
- **T1 批 1 入库 20 条**（scripts/bazi-t1-batch1.py，SRC-ZIPINGZHENQUAN-T1B1 / trust 0.85 / KB-BZ1 前缀）：用神正解、善恶用神顺逆用、相神、成败救应四层、用神纯杂、五合争妒、得禄旺衰锚点、三合三会、八格正名、正官/财/印/食神/七杀/伤官/阳刃/建禄八格深论、外格取舍、大运盖头截脚、宫位限运——每条标注《子平真诠》具体篇目出处，定义级内容非应用句。
- **五红线全检**：长度≥60 + 主题锚点（词表扩至命理核心 26 词）+ 全库指纹幂等 + 出处标注 + 守卫同步 fts5。
- **检索验收（trigram tokenizer 特性留证）**：3 字以上词条正常命中（「相神有力」「格局成败救应」均返回 KB-BZ1 行）；2 字查询「相神」为 trigram 已知限制（≥3 字符），非索引缺失——检索使用时按 3 字词组习惯。
- **终态**：主表=fts5=316,803，bazi 域 603→623，五红线全绿，health-patrol 全绿。
