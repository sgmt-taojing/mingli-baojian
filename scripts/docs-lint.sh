...
# 2. 内部链接失效检测
LINKS_BROKEN=$(grep -rEn '\]\(\.\.?/[^)]+\.md\)' "$DOCS_DIR" 2>/dev/null | awk -F'\\]\\(' '{for(i=2;i<=NF;i++){print $i}}' | grep -oE '\\.?/[^)]+\\.md' | sed 's/^\\.\\///;s/^\\///' | sort -u | while read p; do
  base=$(basename "$p")
  if [ ! -f "$DOCS_DIR/$p" ] && [ ! -f "$DOCS_DIR/$base" ]; then
    echo "$p"
  fi
done | wc -l | tr -d ' ')

# 3. 孤立文件（无任何被引用；README.md 为索引根文件，豁免）
ORPHAN_TOTAL=0
