...
# 2b) medical-stack-page-follow.py：tcm 页面增量 → medical-stack 页面层重打包（链4扩展，内容幂等）
...
FOLLOW_RC=$?
/usr/bin/python3 "$BASE/medical-stack-page-follow.py"
PAGE_RC=$?
...
[ $IMPORT_RC -eq 0 ] && [ $FOLLOW_RC -eq 0 ] && [ $PAGE_RC -eq 0 ] && [ $PARITY_RC -eq 0 ] && [ $DIFF_RC -eq 0 ]
