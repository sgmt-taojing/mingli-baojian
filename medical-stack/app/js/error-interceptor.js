...
(function (global) {
  ...
  const CONFIG = {
    ...
    // R730 修真——SOS 触发白名单：只有真正的紧急事件端点才触发推送
    // 避免 stub 端点（如占位 /api/.../feedback）返回的 {ok:false,error:'处方不存在'} 误触发 SOS
    sosTriggerPaths: ['/api/home/sos', '/api/wechat/sos', '/api/emergency/sos'],
    // R730 修真——SOS 触发码白名单：业务码 0 (成功) 之外的紧急类
    sosTriggerCodes: [401001, 401002, 500001, 503001, 503002, 504000]
  };
        ...
        return p.then((resp) => {
          ...
          if (ct.includes('application/json')) {
            return resp.clone().json().then((body) => {
              const n = normalizeResponse(resp, body); // R729 修真：原参数错位 (body, status) 致 data=httpStatus
              // HTTP 错误覆盖业务码（4xx/5xx）
              if (!resp.ok && n.code === 0) {
                n.code = httpCodeToBiz(resp.status);
              }
              // R729 修真：补 .json()/.text() 兼容方法——全站 25 页 100+ 处 r.json() 曾静默抛错
              const wrapped = { ok: n.code === 0, code: n.code, message: n.message, data: n.data, traceId: n.traceId, httpStatus: resp.status, response: resp };
              wrapped.json = function () { return Promise.resolve(n.data !== null && n.data !== undefined ? n.data : body); };
              wrapped.text = function () { return Promise.resolve(JSON.stringify(n.data !== null && n.data !== undefined ? n.data : body)); };
              return wrapped;
            }).catch((e) => {
              const wrappedErr = { ok: false, code: ERROR_CODES.PARSE_ERROR, message: t('error.504003', 'JSON 解析失败'), data: null, httpStatus: resp.status, response: resp };
              wrappedErr.json = function () { return Promise.resolve(null); };
              wrappedErr.text = function () { return Promise.resolve(''); };
              return wrappedErr;
            });
          }
          // 非 JSON：HTTP 状态判断
  ...
  // R730 修真——SOS 智能触发：只在白名单端点 + 紧急码才触发推送
  function shouldTriggerSOS(result, ctx) {
    if (!result || result.ok) return false;
    const url = (ctx && ctx.url) || '';
    const code = result.code;
    // 条件 1：URL 命中 SOS 端点白名单
    const urlMatch = CONFIG.sosTriggerPaths.some(p => url.includes(p));
    // 条件 2：业务码在紧急类白名单
    const codeMatch = CONFIG.sosTriggerCodes.includes(code);
    // 必须两个条件同时满足（防止 stub 404 误触发）
  ...
  function handleBizError(result, ctx) {
    ...
    // R730 修真——SOS 触发判断（白名单 + 紧急码双校验）
    if (shouldTriggerSOS(result, ctx)) {
      try {
        emit('sos', { code, message: result.message, url: ctx && ctx.url, method: ctx && ctx.method });
      } catch (_) {}
    }
        ...
        xhr.addEventListener('loadend', function () {
          try {
            ...
            if (ct.includes('application/json')) {
              ...
              if (body) {
                const n = normalizeResponse(xhr, body); // R729 修真：同款参数错位
                if (!n.code) { /* 成功不处理 */ }
